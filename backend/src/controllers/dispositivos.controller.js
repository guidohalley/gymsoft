...
import {getAll as getAllDispositivos} from '../services/dispositivos.service.js'
import { validationResult } from 'express-validator';
import { mensajeError, mensajeExito } from '../utils/responseHandler.util.js';
import HTTP_STATUS from '../constants/httpStatusCodes.js';

const __fileName = 'dispositivos.controller.js';
const model = 'Dispositivo';
...
const getAll = async (req, res,next) => {
    try {
        const { gimnasioId } = req.payload;
        const { nombre, activo,page = 1, limit = 10 } = req.query;

        const datos = await getAllDispositivos(model,gimnasioId,{
            nombre: nombre,
...
const update = async (req, res,next) => {
    ...
    try {
        ...
        const datoActualizado = await commonService.update(model,id,{nombre,activo,descripcion,claseId });
